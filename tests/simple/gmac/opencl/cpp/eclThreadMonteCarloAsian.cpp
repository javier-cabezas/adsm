...
#include "../eclThreadMonteCarloAsianKernel.cl"
...
ecl::error error_code = eclSuccess;
...
void* MonteCarloAsian(void *ThreadID)
{
	cl_uint *randNum = NULL;                   /**< Array of random numbers */
	gmactime_t s, t;
	unsigned * n = (unsigned *)ThreadID;
	...
	// Call the kernel
	getTime(&s);
	ecl::config globalThreads(width, height);
	ecl::config localThreads(blockSizeX, blockSizeY);
	ecl::kernel kernel("calPriceVega", error_code);
	assert(error_code == eclSuccess);
#ifndef __GXX_EXPERIMENTAL_CXX0X__
	error_code = kernel.setArg(2, width);
	assert(error_code == eclSuccess);
	error_code = kernel.setArg(1, noOfSum);
	assert(error_code == eclSuccess);
	error_code = kernel.setArg(3, randNum);
	assert(error_code == eclSuccess);
	error_code = kernel.setArg(4,  priceVals[*n]);
	assert(error_code == eclSuccess);
	error_code = kernel.setArg(5, priceDeriv[*n]);
	assert(error_code == eclSuccess);
	...
	for(int k = 0; k < steps; k++) {
		...
		error_code = kernel.setArg(0, attributes);
		assert(error_code == eclSuccess);
		error_code = kernel.callNDRange(globalThreads, localThreads);
		assert(error_code == eclSuccess);
#else
	error_code = kernel(globalThreads, localThreads)(attributes,  noOfSum, width, randNum, priceVals[*n], priceDeriv[*n]);
	assert(error_code == eclSuccess);
#endif
	/* Replace Following "for" loop with reduction kernel */
	for(int i = 0; i < noOfTraj * noOfTraj; i++) {
		price[*n][k] += priceVals[*n][i];
		vega[*n][k] += priceDeriv[*n][i];
	}
	price[*n][k] /= (noOfTraj * noOfTraj);
	vega[*n][k] /= (noOfTraj * noOfTraj);

	price[*n][k] = exp(-interest * maturity) * price[*n][k];
	vega[*n][k] = exp(-interest * maturity) * vega[*n][k];
}
getTime(&t);
printTime(&s, &t, "\nRun: ", "\n");
...
int main(int argc, char *argv[])
{
	...
	error_code = ecl::compileSource(code);
	assert(error_code == eclSuccess);
	...
	assert(sigma != NULL);
	price = new cl_float*[nIter];
	vega = new cl_float*[nIter];
	refPrice = new cl_float*[nIter];
	refVega = new cl_float*[nIter];
	priceVals = new cl_float*[nIter];
	priceDeriv = new cl_float*[nIter];
	for(unsigned n = 0; n < nIter; n++) {
		price[n] = (cl_float*) malloc(steps * sizeof(cl_float));
		assert(price != NULL);
		vega[n] = (cl_float*) malloc(steps * sizeof(cl_float));
		assert(vega != NULL);
		refPrice[n] = (cl_float*) malloc(steps * sizeof(cl_float));
		assert(refPrice != NULL);
		refVega[n] = (cl_float*) malloc(steps * sizeof(cl_float));
		assert(refVega != NULL);
		priceVals[n] = new (ecl::allocator) cl_float[width * height * 2 * sizeof(cl_float4)];
		assert(priceVals[n] != NULL);
		priceDeriv[n] = new (ecl::allocator) cl_float[width * height * 2 * sizeof(cl_float4)];
		assert(priceDeriv[n] != NULL);
	}
	getTime(&t);
	printTime(&s, &t, "Alloc: ", "\n");
	...
	for (unsigned i = 0; i < nIter; i++) {
		memset((void*)price[i], 0, steps * sizeof(cl_float));
		memset((void*)vega[i], 0, steps * sizeof(cl_float));
		memset((void*)refPrice[i], 0, steps * sizeof(cl_float));
		memset((void*)refVega[i], 0, steps * sizeof(cl_float));
		ecl::memset(priceVals[i], 0, width * height * 2 * sizeof(cl_float4));
		ecl::memset(priceDeriv[i], 0, width * height * 2 * sizeof(cl_float4));
	}
	getTime(&t);
	printTime(&s, &t, "Init: ", "\n");
	...
	unsigned *ThreadID = (unsigned *)malloc(nIter * sizeof(unsigned));

	for(unsigned n = 0; n < nIter; n++) {
		ThreadID[n] = n;
		nThread[n] = thread_create(MonteCarloAsian, &ThreadID[n]);
	}
	...
	getTime(&s);
	for (unsigned n = 0; n < nIter; n++) {
		cpuReferenceImpl(sigma, refPrice[n], refVega[n], priceVals[n], priceDeriv[n]);
		/* compare the results and see if they match */
		for(int i = 0; i < steps; ++i) {
			if(fabs(price[n][i] - refPrice[n][i]) > 0.2f)
			{
				printf( "\nFailed\n");
			}
			if(fabs(vega[n][i] - refVega[n][i]) > 0.2f)
			{
				printf( "\nFailed\n");
			}
		}
	...
	for(unsigned n = 0; n < nIter; n++) {
		...
		free(refVega);
		refVega = NULL;
		error_code = ecl::free(priceVals[n]);
		assert(error_code == eclSuccess);
		error_code = ecl::free(priceDeriv[n]);
		assert(error_code == eclSuccess);
	}
	if(price)
	{
