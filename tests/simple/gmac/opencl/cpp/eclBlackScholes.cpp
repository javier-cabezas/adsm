...
const cl_uint widthDefault = 64;
cl_uint width = widthDefault;
...
void blackScholesCPU(cl_float *randArray, cl_int width, cl_int height, cl_float *hostCallPrice, cl_float *hostPutPrice)
{
    int y;
...
int main(int argc, char *argv[])
{
    ...
    ecl::error err;
    ecl::kernel kernel("blackScholes", err);
    assert(err == eclSuccess);
#ifndef __GXX_EXPERIMENTAL_CXX0X__
    assert(kernel.setArg(0, randArray) == eclSuccess);
    assert(kernel.setArg(1, width) == eclSuccess);
    assert(kernel.setArg(2, deviceCallPrice) == eclSuccess);
    assert(kernel.setArg(3, devicePutPrice) == eclSuccess);
    assert(kernel.callNDRange(globalSize, localSize) == eclSuccess);
#else
    assert(kernel(globalSize, localSize)(randArray, width, deviceCallPrice, devicePutPrice) == eclSuccess);
#endif
    getTime(&t);
    printTime(&s, &t, "Run: ", "\n");
