...
int main(int argc, char *argv[])
{
	...
	assert(kernel.setArg(2, width) == eclSuccess);
	assert(kernel.setArg(1, noOfSum) == eclSuccess);
	assert(kernel.setArg(3, randNum) == eclSuccess);
	assert(kernel.setArg(4, priceVals) == eclSuccess);
	assert(kernel.setArg(5, priceDeriv) == eclSuccess);
...
#else
	assert(kernel(globalSize, localSize)(attributes,  noOfSum, width, randNum, priceVals, priceDeriv) == eclSuccess);
#endif
	/* Replace Following "for" loop with reduction kernel */
	for(int i = 0; i < noOfTraj * noOfTraj; i++) {
		price[k] += priceVals[i];
