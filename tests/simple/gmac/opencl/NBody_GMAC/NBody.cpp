...
NBody::setupNBody()
{
    ...
    assert(eclMalloc((void **) &currPos, numBodies * sizeof(cl_float4)) == eclSuccess);
    assert(eclMalloc((void **) &currVel, numBodies * sizeof(cl_float4)) == eclSuccess);

    assert(eclMalloc((void **) &newPos, numBodies * sizeof(cl_float4)) == eclSuccess);
    assert(eclMalloc((void **) &newVel, numBodies * sizeof(cl_float4)) == eclSuccess);
    ...
    memcpy(initPos, currPos, 4 * numBodies * sizeof(cl_float));
    memcpy(initVel, currVel, 4 * numBodies * sizeof(cl_float));
...
NBody::setupCL()
{
    assert(eclCompileSourceFile("NBody_Kernels.cl") == eclSuccess);

    assert(eclGetKernel("nbody_sim", &kernel) == eclSuccess);
...
NBody::setupCLKernels()
{
    ...
    /* Particle positions */
    assert(eclSetKernelArgPtr(kernel, 0, currPos) == eclSuccess);
    /* Particle velocity */
    assert(eclSetKernelArgPtr(kernel, 1, currVel) == eclSuccess);
    /* numBodies */
    assert(eclSetKernelArg(kernel, 2, sizeof(cl_int), &numBodies) == eclSuccess);
    /* time step */
    assert(eclSetKernelArg(kernel, 3, sizeof(cl_float), &delT) == eclSuccess);
    /* upward Pseudoprobability */
    assert(eclSetKernelArg(kernel, 4, sizeof(cl_float), &espSqr) == eclSuccess);
    /* local memory */
    assert(eclSetKernelArg(kernel, 5, GROUP_SIZE * 4 * sizeof(cl_float), NULL) == eclSuccess);
    /* Particle positions */
    assert(eclSetKernelArgPtr(kernel, 6, newPos) == eclSuccess);
    /* Particle velocity */
    assert(eclSetKernelArgPtr(kernel, 7, newVel) == eclSuccess);
...
NBody::runCLKernels()
{
    ...
    */
    ...
    eclMemcpy(currPos, newPos, sizeof(cl_float4) * numBodies);
    eclMemcpy(currVel, newVel, sizeof(cl_float4) * numBodies);
    return 0;
}
...
NBody::cleanup()
{
    ...
    assert(eclReleaseKernel(kernel) == eclSuccess);
    assert(eclFree(currPos) == eclSuccess);
    assert(eclFree(currVel) == eclSuccess);
    assert(eclFree(newPos) == eclSuccess);
    assert(eclFree(newVel) == eclSuccess);
