...
TEST_F(ContextTest,ContextMemory){
	...
	memset(buffer2, 0, Size_ * sizeof(int));
	memset(canary,  0, Size_ * sizeof(int));
	ASSERT_TRUE(memcmp(buffer,canary,Size_ * sizeof(int)) == 0);

	accptr_t dstdevice=NULL;
	ASSERT_TRUE(GetAccelerator().map(dstdevice, hostptr_t(buffer2), Size_*sizeof(int))==gmacSuccess); //device
	...
	ASSERT_TRUE(GetAccelerator().unmap(hostptr_t(buffer2), Size_ * sizeof(int)) == gmacSuccess);
	delete[] canary;
	delete[] buffer;
	delete[] buffer2;
}
