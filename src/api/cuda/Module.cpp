    ...
    for (it = dVector.begin(); it != dVector.end(); it++) {
        ...
        for (v = d.variables_.begin(); v != d.variables_.end(); v++) {
            ...
            if (f != variablesByName_.end()) {
                ...
                variablesByName_.insert(VariableNameMap::value_type(std::string(v->getName()), Variable(*v, mod_)));
            }
        }
        ...
        for (v = d.constants_.begin(); v != d.constants_.end(); v++) {
            ...
            constantsByName_.insert(VariableNameMap::value_type(std::string(v->getName()), Variable(*v, mod_)));
        }
