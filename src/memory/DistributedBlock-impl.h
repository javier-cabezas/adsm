...
template<typename State>
inline core::Mode &DistributedBlock<State>::owner() const
{
	return core::Mode::getCurrent();
}

template<typename State>
inline accptr_t DistributedBlock<State>::acceleratorAddr(const hostptr_t addr) const
{
...
template<typename State>
inline accptr_t DistributedBlock<State>::acceleratorAddr() const
{
	accptr_t ret = accptr_t(0);
