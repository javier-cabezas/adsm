...
static LONG CALLBACK segvHandler(EXCEPTION_POINTERS *ex)
{
    ...
    bool resolved = false;
    core::Mode *mode = Process_->get_owner((host_const_ptr)addr);
    if(mode != NULL) {
        if(!writeAccess) resolved = Manager_->signal_read(*mode, (host_ptr)addr);
        else             resolved = Manager_->signal_write(*mode, (host_ptr)addr);
    }
