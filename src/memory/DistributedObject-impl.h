    ...
    Object(cpuAddr, size)
{
    ...
    TRACE(LOCAL, "Creating Distributed Object @ %p : shadow @ %p) ", 
        addr_, shadow_);
}
...
template<typename State>
inline accptr_t DistributedObject<State>::acceleratorAddr(const hostptr_t addr) const
{
...
template<typename State>
inline core::Mode &DistributedObject<State>::owner(const hostptr_t addr) const
{
	lockRead();
	BlockMap::const_iterator i = blocks_.upper_bound(addr);
