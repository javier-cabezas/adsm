...
lazy_base::signal_read(block_ptr_impl _block, host_ptr addr, gmacError_t &err)
{
    ...
    event_ptr_impl ret = parent::signal_read(block, addr, err);
...
lazy_base::signal_write(block_ptr_impl _block, host_ptr addr, gmacError_t &err)
{
    ...
    event_ptr_impl ret = parent::signal_write(block, addr, err);

    ENSURES(block->get_state() == state_impl::Dirty);
...
lazy_base::acquire(block_ptr_impl _block, GmacProtection prot, gmacError_t &err)
{
	...
	lazy_block_ptr_impl block = __impl::util::static_pointer_cast<lazy_block_impl>(_block);
    event_ptr_impl ret = parent::acquire(block, prot, err);

    ENSURES(err == gmacSuccess);
...
lazy_base::release(block_ptr_impl _block, gmacError_t &err)
{
	...
	lazy_block_ptr_impl block = __impl::util::static_pointer_cast<lazy_block_impl>(_block);
    event_ptr_impl ret = parent::release(block, err);

    ENSURES(block->get_state() == state_impl::ReadOnly ||
            block->get_state() == state_impl::Invalid);
...
{
    event_ptr_impl ret = parent::release_all(err);
...
lazy_base::map_to_device(block_ptr_impl _block, gmacError_t &err)
{
	REQUIRES(bool(_block));
	lazy_block_ptr_impl block = __impl::util::static_pointer_cast<lazy_block_impl>(_block);
	event_ptr_impl ret = parent::map_to_device(block, err);
...
lazy_base::unmap_from_device(block_ptr_impl _block, gmacError_t &err)
{
	REQUIRES(bool(_block));
	lazy_block_ptr_impl block = __impl::util::static_pointer_cast<lazy_block_impl>(_block);
	event_ptr_impl ret = parent::unmap_from_device(block, err);

	ENSURES(block->get_state() == state_impl::Dirty ||
			block->get_state() == state_impl::ReadOnly);
...
{
	REQUIRES(bool(block));
	event_ptr_impl ret = parent::remove_block(block, err);
...
lazy_base::to_host(block_ptr_impl _block, gmacError_t &err)
{
	REQUIRES(bool(_block));
	lazy_block_ptr_impl block = __impl::util::static_pointer_cast<lazy_block_impl>(_block);
    event_ptr_impl ret = parent::to_host(block, err);

    ENSURES(block->get_state() != state_impl::Invalid);
...
lazy_base::memset(const block_ptr_impl block, size_t blockOffset, int v, size_t size, gmacError_t &err)
{
    ...
    event_ptr_impl ret = parent::memset(block, blockOffset, v, size, err);
...
{
    event_ptr_impl ret = parent::flush_dirty(err);
...
lazy_base::copy_block_to_block(block_ptr_impl d, size_t dstOffset, block_ptr_impl s, size_t srcOffset, size_t count, gmacError_t &err)
{
    ...
    event_ptr_impl ret = parent::copy_block_to_block(d, dstOffset, s, srcOffset, count, err);
