...
{
    if (devNum < 0 || unsigned(devNum) >= __impl::core::hpe::get_process().get_resource_manager().get_number_of_devices())
        return cudaErrorInvalidDevice;
		...
		size_t height, enum cudaMemcpyKind kind)
{
    ...
    __impl::memory::address_space_ptr aspace = manager.get_owner(host_ptr(src));
    if(aspace == NULL) {
#if CUDA_VERSION >= 3020
