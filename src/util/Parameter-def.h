...
PARAM(paramModelToHostTransferL1, float, 0.0007, "GMAC_MODEL_TOHOSTTRANSFER_L1") // Transfer costs for data that fits in the L1 cache
PARAM(paramModelToHostTransferL2, float, 0.0008, "GMAC_MODEL_TOHOSTTRANSFER_L2") // Transfer costs for data that fits in the L2 cache
PARAM(paramModelToHostTransferMem, float, 0.0010, "GMAC_MODEL_TOHOSTTRANSFER_L2") // Transfer costs for data that does not fit in the L2 cache
PARAM(paramModelToDeviceConfig, float, 40.0, "GMAC_MODEL_TODEVICECONFIG")       // DMA configuration costs
PARAM(paramModelToDeviceTransferL1, float, 0.0007, "GMAC_MODEL_TODEVICETRANSFER_L1") // Transfer costs for data that fits in the L1 cache
PARAM(paramModelToDeviceTransferL2, float, 0.0008, "GMAC_MODEL_TODEVICETRANSFER_L2") // Transfer costs for data that fits in the L2 cache
PARAM(paramModelToDeviceTransferMem, float, 0.0010, "GMAC_MODEL_TODEVICETRANSFER_L2") // Transfer costs for data that does not fit in the L2 cache
